...
void setup()
{
  ...
  {
    //shifts[i]*=timestep;
    servo[i].write(amp*sin(shifts[i])+startPos);
    servo[i].attach(i+2);
    if (periods[i] != 0)
    {
      ...
      minPeriodArr[i] = abs((1.0-mutate)*float(periods[i])*float(timestep));
      maxPeriodArr[i] = abs((1.0+mutate)*float(periods[i])*float(timestep));
    }
    else
    {
    ...
    while (positions[ser][N-1]==0)
    {
      ...
      if (lastWrite[ser][0]==0)
      {
        ...
        else
        {
          ...
          //{toPhase = -1;}
          //else {toPhase = 1;};
          //if (periods[ser]<0){toPhase=-1;periods[ser]=abs(periods[ser]);};
      ...
      //Serial.print(num);
      //Serial.print(", ");
      //Serial.print(toPhase);
      //Serial.print("\n");
      for (int j=0;j<n;j++)
      {
        float J = j;
        int val = phase*amp*sin(pi*2*J*timestep/period+shift)+startPos;
        //int val = amp*sin(pi*2*J*timestep/period+shift)+startPos;
